...
public class SearchWindow extends JFrame {
	...
	static JTextField passNumbTF;
	private JLabel originLB, destinationLB, dateLB, passNumbLB ;
	private JTextField originTF, destinationTF, dateTF;
	...
	public SearchWindow(Controller controller) {
		...
		passNumbLB = new JLabel("Number of seats");
		passNumbLB.setBounds(20, 170, 100, 40);
	...
	private void searchBtnActionPerformed() {
		...
		if(search.size() != 0) {
			...
			for(EBFlightDTO f : search) {
				System.out.println(f.getOrigin() + ", " + f.getDestination() + ", " + f.getDate() + ", " + f.getPrice());
			}
			setVisible(false);
